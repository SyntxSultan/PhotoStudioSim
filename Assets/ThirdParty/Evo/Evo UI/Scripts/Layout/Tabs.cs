...
namespace Evo.UI
{
    ...
    public class Tabs : MonoBehaviour
    {
        ...
        void OnDisable()
        {
            ...
            if (indicatorCoroutine != null) { StopCoroutine(indicatorCoroutine); indicatorCoroutine = null; }
        }
        ...
        IEnumerator SwitchTab(int newTabIndex, bool animate = true)
        {
            ...
            // Perform animation (yielding pure Enumerators directly to avoid StartCoroutine GC allocations)
            if (animate) { yield return AnimateTabTransition(currentTab, newTab, newTabIndex); }
            else { yield return AnimateNone(currentTab, newTab); }
        ...
        IEnumerator AnimateTabTransition(Item currentTab, Item newTab, int newTabIndex)
        {
            switch (animationType)
            {
                case AnimationType.None:
                    yield return AnimateNone(currentTab, newTab);
                    break;
        ...
        {
            if (tabs.Count == 0 || upcomingTabIndex == tabIndex || tabs[tabIndex].tabObject == null)
                return;
